...
## ⚠️UPDATE
Я решил попробовать переделать ДЗ с моделью XGBoost, которая точно поддерживается ML-flow и отлично в нем логируется. На этот раз мы будем предсказывать фракцию сердечного выброса (`ejection_fraction`) по косвенным признакам. Эксперименты я решил [разместить в другой ветке](https://github.com/Aeverandi/mipt_ml-production_hw/tree/new_ef_predict).
